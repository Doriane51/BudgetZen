...
export default function BudgetForm({ open, onClose }) {
  const { addBudget, budgets, activeMonth } = useStore()
  ...
  const handleSubmit = () => {
    ...
    // Vérifier si un budget existe déjà pour cette catégorie ce mois
    const exists = budgets.find(
      (b) => b.category === form.category && b.month === activeMonth
    )
    ...
    addBudget({ ...form, limit: Number(form.limit), month: activeMonth })
    setForm(defaultForm)
    setError('')
    onClose()
  }
